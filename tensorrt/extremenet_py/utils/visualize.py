...
def vis_mask(img, mask, alpha=0.4, show_border=True, border_thick=1):
...
def vis_octagon(img, extreme_points, is_ver_or_hor, border_thick=2):
    """Visualizes a single binary mask."""
    ...
    mask = extreme_point_to_octagon_mask(
      extreme_points, img.shape[0], img.shape[1], is_ver_or_hor)

    img = vis_mask(img, mask)
...
def get_octagon(ex, is_ver_or_hor):
...
def extreme_point_to_octagon_mask(extreme_points, h, w, is_ver_or_hor):
  octagon = get_octagon(extreme_points, is_ver_or_hor)
